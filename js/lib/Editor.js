...
G.def(
    'Editor',
    ['UBBUtils', 'Overlay', 'ajaxfile', 'tab', 'EditorHelper', 'cookies', 'rangy-restore', 'TextUtils', 'at', 'autoHeight'],
    function(UBBUtils, Overlay, ajaxfile, tab, EditorHelper, cookies, rangy, TextUtils, at, autoHeight) {
        'use strict';
        var idNum  = 0,
            cookie = 'editorMod',
        ...
        return function(option) {
            ...
            var _option = {
                    ...
                    initMod: parseInt(cookies.get(cookie), 10) || 0 // 编辑器的初始化模式，优先使用配置代码，其次为cookies中的editorMod值，最后为0
                },
                ...
                iframeTxt = '<iframe id="{id}" class="gui-ubb-editor" frameborder="0"></iframe>', // iframe的html模板
                ...
                // NOT IE
                    ...
                    if (mod) {
                        ...
                        if (!G.ua.isOpera) {
                            ...
                            if (divRange) {
                                rangy.restoreSelection(divRange, true);
                            }
                            divRange = null;
                        }
                    } else {
                        if (textareaRange) {
                            $textarea.setSelection(textareaRange.start, textareaRange.end);
                        }
                    }
                }
             }
            ...
            function showAtBlock(cb) {
                var atPos, pos;
                if (mod) {
                    atPos = rangy.getCaretPosition(win);
                    if (atPos) {
                        pos = $iframe.offset();
                        // 确保编辑器高度改变时候依旧ok
                        atPos.top += pos.top;
                    ...
                    if (sel.length === 0) {
                        pos = $textarea.offset();
                        var start = txt.slice(0, sel.start),
            ...
            function updateHeight() {
                if (mod && iframeAutoHeight) {
                    iframeAutoHeight.adjustHeight();
                } else if (textAutoHeight) {
                    textAutoHeight.adjustHeight();
                }
            }
            ...
            function init() {
                ...
                // 初始化自动增高
                // setTimeout to fix weird bug on IE9
                setTimeout(function() {
                    textAutoHeight = autoHeight($textarea);
                    if (supportView()) {
                        iframeAutoHeight = autoHeight($iframe);
                    }
                });
            }
             ...
             * 进入可视化编辑模式
             */
            this.showEditor = function(notFocus) {
                showEditor(notFocus);
                return this;
            };
            /**
             * 进入源码编辑模式
             */
            this.showSource = function(notFocus) {
                showSource(notFocus);
                return this;
            };
            this.clear = function() {
            ...
            this.remove = function() {
                textAutoHeight.destroy();
                if(iframeAutoHeight) {
                    iframeAutoHeight.destroy();
                }
                $actionPanel.remove();
                if (oldText == null) {
                    $borderGap.parent()
